...
def annotate_hormone_long(
    ...
    hormone_ct_long = pd.merge(
        hormone_long,
        hormone_producing.loc[:, ['hormone_display','hormone_short','Tier']],
        left_on='Hormone',
...
def calculate_hormone_strength(
    ...
    use_precomputed:Optional[str] = None,
    max_expression_file='max_expression_final.csv'
) -> pd.DataFrame:
    """
    ...
    max_expression.to_csv(max_expression_file)
...
def calculate_hormone_strength_specificity(
    ...
    cellanno = (
        ave_all.loc[:, [celltype_column, tissue_col]]
        .drop_duplicates(subset=[celltype_column])
        .reset_index(drop=True)
    )
    ...
    if (hormone_long_adj_coverage is None ) and (hormone_long_adj_specificity is None):
        ...
        ## replace the hormone_all using its offspring HPC
        hormone_member_to_all = {
            'activin_a': 'activins_all',
            'activin_ab': 'activins_all',
            'activin_b': 'activins_all',
            'activin_e': 'activins_all',
            ...
            'endothelin_1': 'endothelins_all',
            'endothelin_2': 'endothelins_all',
            'endothelin_3': 'endothelins_all',
        
            'inhibin_a': 'inhibin_all',
            'inhibin_b': 'inhibin_all',
        
            'anp': 'natriuretic_peptides_all',
            'bnp': 'natriuretic_peptides_all',
            'cnp': 'natriuretic_peptides_all',
        
            'prokineticin_1': 'prokineticin_all',
            'prokineticin_2': 'prokineticin_all',
        
            'relaxin__relaxin_2': 'relaxin_like_all',
            'relaxin_like__relaxin_1': 'relaxin_like_all',
            'relaxin_like__relaxin_3': 'relaxin_like_all',
            'relaxin-like__insl3': 'relaxin_like_all',
            'relaxin-like__insl4': 'relaxin_like_all',
            'relaxin-like__insl5': 'relaxin_like_all',
            'relaxin-like__insl6': 'relaxin_like_all',
        
            'urocortin_1': 'urocortins_all',
            'urocortin_2': 'urocortins_all',
            'urocortin_3': 'urocortins_all'
        }
        # replace the offspring using hormone_all
        mask=hormone_long_new_replace['Hormone'].isin(hormone_member_to_all.keys())
        hormone_long_new_replace1=hormone_long_new_replace.loc[mask].copy()
        hormone_long_new_replace1['Hormone']=hormone_long_new_replace1['Hormone'].map(hormone_member_to_all)
        # merge the new data with hpc list without hormone_all
        mask=hormone_long_new_replace['Hormone'].isin(hormone_member_to_all.values())
        ...
        hormone_prohormone_dict = {
            # "agrp": "pro_agrp",
            "amylin_iapp": "pro_amylin_iapp",
            "avp": "pro_avp",
            "cart": "pro_cart",
            "cck": "pro_cck",
            "dynorphin": "pro_dynorphin",
            "enkephalin": "pro_enkephalin",
            "gastrin": "pro_gastrin",
            "ghrelin_acylated": "pro_ghrelin",
            "gip": "pro_gip",
            "glucagon": "pro_glucagon",
            "insulin": "pro_insulin",
            "neuropeptide_y": "pro_npy",
            "somatostatin": "pro_somatostatin",
            "trh": "pro_trh",
            "acth": "pomc",
            'pomc_acth_1_17':'pomc',
            'pomc_aMSH':'pomc',
            'pomc_beta_endorphin':'pomc',
            'pomc_bMSH':'pomc',
            'pomc_gMSH':'pomc',
            "crh": "pro_crh", #added in v1.0.5
            "glp1":"pro_glucagon",
            "glp2":"pro_glucagon"
        }
        # replace the mature hormone using the intersection
        # replace the mature hormone using the intersection
        mature_hormones=list(set(hormone_prohormone_dict.keys()).intersection(set(hormone_long_new_replace['Hormone'])))
        holder_mature = []
        for hormone in mature_hormones:#hormone_prohormone_dict.keys():
            # Find rows for the mature hormone
            mask = hormone_long_new_replace['Hormone'].isin([hormone])
            hormone_long_new_replace1 = hormone_long_new_replace.loc[mask].copy()
        ...
        if holder_mature:
            hormone_long_new_replace_mature = pd.concat(holder_mature, axis=0, ignore_index=True)
        else:
            hormone_long_new_replace_mature = pd.DataFrame(columns=hormone_long_new_replace.columns)
        
        # Get rows that are not mature hormones (not in dictionary keys)
        mask = ~hormone_long_new_replace['Hormone'].isin(hormone_prohormone_dict.keys())
        hormone_long_new_replace2 = hormone_long_new_replace.loc[mask].copy()
        
        # Combine the filtered mature hormone data with other hormone data
        hormone_long_new_replace = pd.concat([hormone_long_new_replace_mature, hormone_long_new_replace2], ignore_index=True)
        hormone_long_new_replace = hormone_long_new_replace.drop_duplicates()
        
        # print(f"Number of mature hormone intersection rows: {len(hormone_long_new_replace_mature)}")
        # print(f"Number of other hormone rows: {len(hormone_long_new_replace2)}")
        # print(f"Total rows after combination: {len(hormone_long_new_replace)}")
        ...
        # holder_mature=[]
        # for hormone in hormone_prohormone_dict.keys():
        #     mask=hormone_long_new_replace['Hormone'].isin(hormone)
        #     hormone_long_new_replace1=hormone_long_new_replace.loc[mask].copy()
        #     mask=hormone_long_new_replace['Hormone'].isin(hormone_prohormone_dict[hormone])
        #     hormone_long_new_replace2=hormone_long_new_replace.loc[mask].copy()
        #     mask=hormone_long_new_replace1['Celltype_tissue'].isin(hormone_long_new_replace2['Celltype_tissue'])
        #     pairs=hormone_long_new_replace1.loc[mask]
        #     holder_mature.append(pairs)

        # hormone_long_new_replace_mature=pd.concate(holder_mature,axis=0)

        # mask=hormone_long_new_replace['Hormone'].isin(hormone_prohormone_dict.keys())
        # hormone_long_new_replace2=hormone_long_new_replace.loc[~mask].copy()
        # hormone_long_new_replace=pd.concat([hormone_long_new_replace_mature,hormone_long_new_replace2])
        # hormone_long_new_replace = hormone_long_new_replace.drop_duplicates()
